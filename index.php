<?php
/*
  Plugin Name: Kuratur Connect 3
  Description: The new improved Kuratur. Preview your Kuratur feeds before they publish to your site. Delete posts that you don't want. 
  
  The painless way to always have fresh relevant content on your blog. Curate a blog page of automated content using your favorite social media sources.
  Version: 2.0
			...
			<div class="kuratur_introduction" style="padding:30px 20px;text-align:center;"><br>
				...
				<form action="$createPath" method="post" id="import_content_from_kuratur" style="padding:20px 200px;text-align:left;">
					<p style="font-size: 16px;">
						<label for="kurfeed_name">Please give this Kuratur Feed a name: </label><br><input type="text" id="kurfeed_name" name="kurfeed_name" style="width:500px; height: 30px;" value="" />
					</p>
					<p style="font-size: 16px;">
						<label for="key_api">Key of digest (from Kuratur) : </label><Br> <input type="text" id="key_api" name="key_api" style="width:500px; height: 30px;" value="$keyAPI" />
					</p>
					<p>	<input type="submit" value="Import to WordPress"/></p>
				</form>
			</div>
HEADER;
		} else {		
...
*/		
		...
		if ($kur_update_id === 'false') {
			...
			$kur_post = array(
			  ...
			  'post_title'     => ($_POST['kurfeed_name']), // The title of your post.
...
/*****update existing custom post types hourly using wp_cron******/
...
function kur_cleanup_v1() {
	//check our option to see if we've already done this
	...
	update_option( 'kur_legacy', 'false', '', 'no' );
		
	//if kur_legacy is set to 'false' then nothing to do here
}


/**********************************************************
ADD OUR CUSTOM POST TYPE, METABOXES AND STYLES
**********************************************************/
...
**********************************************************/
function add_kuratur_scripts() {
	wp_register_style( 'kuraturstyles', plugin_dir_url(__FILE__) . 'css/kuratur-feed-styles.css');
	
	wp_enqueue_style( 'kuraturstyles' );
	
	wp_register_script( 'kuraturscripts', plugin_dir_url(__FILE__) . 'js/kuratur-scripts.js');
	
	wp_enqueue_script( 'kuraturscripts' );
}	
add_action( 'wp_enqueue_scripts', 'add_kuratur_scripts' );
...
/**********************************************************
CREATE OUR SHORTCODE FUNCTIONALITY AND ADD THE SHORTCODE
**********************************************************/
...
//make our shortcode work in text widgets
